...
function rachelle_setup() {
	// This theme uses wp_nav_menu() in one location.
	register_nav_menus( array(
		'primary' => esc_html__( 'Primary Menu', 'rachelle' ),
		'footer' => esc_html__( 'Footer Menu', 'rachelle' ),
	) );
}
add_action( 'after_setup_theme', 'rachelle_setup' );

/**
 * Enqueue scripts and styles.
 */
function rachelle_scripts() {
	wp_enqueue_script( 'jquery', get_template_directory_uri() . '/assets/vendor/jquery-2.2.2.min.js', array(), ASSETS_VERSION );
	...
	if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
		...
		wp_enqueue_script( 'rachelle-scripts-dev', get_template_directory_uri() . '/assets/build/scripts.js', array( 'jquery' ), ASSETS_VERSION, true );
	} else {
